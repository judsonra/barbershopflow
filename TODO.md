...
- [ ] **Lacuna ainda não resolvida** (o e-mail do profissional acima é o primeiro passo pra isso, mas falta o resto): cadastrar um profissional só cria a entrada de catálogo — ainda não existe nenhum jeito de dar login pra essa pessoa. Cliente tem o fluxo de celular+SMS (`grantCustomerAccess`); profissional não tem equivalente. Falta um fluxo de convite (ex.: profissional entra com Google/Facebook usando o mesmo e-mail que o gestor cadastrou, e o backend liga a conta ao `professional_id` automaticamente — hoje `findOrCreateSocialUser` só faz esse match contra `users`, não contra `professionals` sem conta ainda)
