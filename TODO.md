...
- [x] Auditoria de impersonate além do log simples atual: toda vez que `POST /admin/tenants/{id}/impersonate` funciona, grava uma linha em `impersonation_audits` (migração `015_impersonation_audit.sql`, gravação best-effort — falha não bloqueia o acesso); `GET /admin/audit` lista a trilha (quem, qual barbearia, quando), mais recente primeiro, limitada a 200 linhas; aba "Auditoria" no `AdminPanel`
...
- [x] Feriados: calendário de dias bloqueados compartilhado pela barbearia inteira (tabela `tenant_holidays`, `GET`/`POST /tenant/holidays`, `DELETE /tenant/holidays/{id}`, só `manager` cria/remove). Nenhum agendamento é aceito na data do feriado, para nenhum profissional (`409 holiday_blocked`) — complementa (não substitui) o bloqueio por profissional já existente em `professional_time_off`. Seção "Feriados" na tela de Config → Agenda
...
- [ ] Ampliar cobertura de testes (hoje cobre pouco além do caminho feliz) — paginação de clientes já ganhou cobertura E2E (`frontend/e2e/customers-pagination.spec.ts`)
- [ ] Estruturar migrações versionadas conforme o schema crescer (tenant, auth, etc.)
- [x] Paginação real (`?page=&limit=`, resposta `{items, total}`) em `GET /customers` — é o único dos três que cresce sem limite de verdade (clientela recorrente acumula com o tempo). `ListServices`/`ListProfessionals` ganharam só um `LIMIT 1000` de segurança no SQL, sem mudança de API: na prática ficam em dezenas por barbearia, e várias telas (dropdown de agendamento, checkboxes de especialidades em Profissionais, filtros da agenda) precisam da lista completa pra funcionar — paginar de verdade quebraria essas telas sem uma reescrita bem maior. A tela de Clientes (Config → Clientes) busca sua própria página, separada da lista completa que a agenda mantém pro dropdown de novo agendamento; criar/editar/(des)ativar um cliente atualiza as duas.
